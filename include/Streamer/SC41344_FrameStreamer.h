 ...
 * - Constants.h: Defines timing constants like FRAME_TRANSMISSION_COUNT
 */
template<size_t N>
...
public:
    ...
    /**
     * @brief Stream the whole code message bit by bit using the Encoder object to generate the waveform.
     *
     * @param code_DataBits Store the code logic bit as an array of '0' and '1'.
     * @param encoder Know how each bit is encoded for the protocol in use.
     */
    void streamFrame(const uint8_t (&code_DataBits)[N], IBitEncoder& encoder) override;

    /**
     * @brief Stream a single frame emission.
     *
     * The transceiver layer can call this repeatedly while cycling the radio
     * between TX and IDLE so every retransmission becomes a distinct RF burst.
     *
     ...
     * @param includePreamble True to emit the sync preamble before the payload.
     */
    ...
    /**
     * @brief Static method to stream the full repeated frame sequence.
     *
     * This method is useful for encoder-only testing because it can generate the
     * complete waveform without needing a Transceiver instance.
     *
     * @param code_DataBits Array of logical bits (0 or 1) representing the frame content.
     * @param encoder Object that encodes individual bits into physical waveforms.
     */
...
/**
 * @brief Streams a complete SC41344 waveform sequence.
 *
 * The sequence is composed of:
 *   1. Preamble (sync pulse)
 *   2. First data word (encoded with encoder)
 *   3. Additional repeated frames, each preceded by a silence gap
 *
...
{
    #if LOG_VERBOSE
        LOG_NEW_LINE("SC41344_FrameStreamer::streamFrame() - Streaming frame sequence...");
    #endif
