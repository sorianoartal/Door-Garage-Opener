...
// The RF output power level used to choose the OOK logic '1' PATABLE byte.
enum class OutputPowerLevel
{
    HIGH_POWER,
    MEDIUM_POWER,
    LOW_POWER
...
struct TransceiverConfig
{
    ...
    TransceiverConfig(uint32_t transmissionFreqBand_Hz = FREQ_315MHZ_BAND, ModulationScheme modulation = ModulationScheme::OOK, OutputPowerLevel powerLevel = OutputPowerLevel::HIGH_POWER);
    ...
    OutputPowerLevel getPowerLevel() const;
    uint8_t getOOKLogicOnePowerByte() const;

    private:

    uint32_t _transmission_frequency_Hz;                // Tran
    ModulationScheme _modulationScheme;
    OutputPowerLevel _powerLevel;
};
